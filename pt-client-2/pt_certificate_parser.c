...
static ce_tlv_status_e take_16bit_number(ce_tlv_element_s *element, uint16_t *number_out)
{
    ...
    *number_out = (uint16_t)ntohs(*number_out);
...
{
    uint16_t num_buf = (uint16_t)htons(number);
...
static void _append_value_bytes(const char *bytes, uint16_t bytes_length, ce_tlv_encoder_s *encoder)
{
    // str_length should contain the '\0'
    memcpy(encoder->buf + encoder->encoded_length, bytes, bytes_length);
    // FIXME: Cast is needed here, need to check why getting compilation warning in Native GCC (Linux)
    encoder->encoded_length = (uint16_t)(bytes_length + encoder->encoded_length);
}
...
ce_tlv_status_e tlv_add_bytes(ce_tlv_type_e type, uint16_t length, const char *value, bool is_tlv_required, ce_tlv_encoder_s *encoder)
{
    ...
    // Append value
    _append_value_bytes(value, length, encoder);
...
ce_tlv_status_e tlv_add_uint16(ce_tlv_type_e type, uint16_t value, bool is_tlv_required, ce_tlv_encoder_s *encoder)
{
    ...
    // Append value
    _append_16bit_number(value, encoder);

...
#ifdef MBED_EDGE_BUILD_CFG_DISABLE_OBS_FEATURE
#define COAP_DISABLE_OBS_FEATURE MBED_EDGE_BUILD_CFG_DISABLE_OBS_FEATURE
...
{
#if defined(MBED_EDGE_BUILD_MBED_CLIENT_C_VERSION_STRING)
    return MBED_EDGE_BUILD_MBED_CLIENT_C_VERSION_STRING;
...
static int8_t sn_nsdl_local_rx_function(struct nsdl_s *handle, sn_coap_hdr_s *coap_packet_ptr, sn_nsdl_addr_s *address_ptr)
{
    ...
    /* Store the current message id and length so that we can identify if same operation was initiated from callback */
    uint16_t temp_msg_id = 0;
    uint16_t temp_msg_len = 0;
    if (is_reg_msg) {
        temp_msg_id = handle->register_msg_id;
        temp_msg_len = handle->register_msg_len;
    }
    else if (is_unreg_msg) {
        temp_msg_id = handle->unregister_msg_id;
    }
    else if (is_update_reg_msg) {
        temp_msg_id = handle->update_register_msg_id;
        temp_msg_len = handle->update_register_msg_len;
    }
    ...
    /* If callback initiated same operation then msg id and msg len are updated in handle and temp values don't match. This means we don't clear them here because we will wait for response to new values. */
    if (is_reg_msg && handle->register_msg_id == temp_msg_id && handle->register_msg_len == temp_msg_len) {
        handle->register_msg_id = 0;
        handle->register_msg_len = 0;
    }
    else if (is_unreg_msg && handle->unregister_msg_id == temp_msg_id) {
        handle->unregister_msg_id = 0;
    }
    else if (is_update_reg_msg && handle->update_register_msg_id == temp_msg_id && handle->update_register_msg_len == temp_msg_len) {

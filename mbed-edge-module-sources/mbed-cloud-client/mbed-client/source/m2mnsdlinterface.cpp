...
bool M2MNsdlInterface::create_nsdl_structure(M2MBase *base)
{
    ...
    if(base) {
        ...
        case M2MBase::Object:
            ...
            break;
        case M2MBase::ObjectInstance:
            success = create_nsdl_object_instance_structure((M2MObjectInstance*)base);
            break;
        case M2MBase::Resource:
            success = create_nsdl_resource_structure((M2MResource*)base, ((M2MResource*)base)->supports_multiple_instances());
            break;
    	}
    }
    return success;
...
bool M2MNsdlInterface::create_nsdl_endpoint_structure(M2MEndpoint *endpoint)
{
    ...
    if(endpoint) {
        ...
        success = create_nsdl_resource(endpoint);
    }
    return success;
}
...
void M2MNsdlInterface::handle_pending_notifications_for_object(bool clear, M2MObject *object)
{
    ...
    if (clear) {
        object->clear_notification_delivery_status();
        // Send all the pending notifications
    } else {
        M2MReportHandler* reporter = object->report_handler();
        if ((object->get_notification_delivery_status() == NOTIFICATION_STATUS_SENT ||
             object->get_notification_delivery_status() == NOTIFICATION_STATUS_SEND_FAILED) &&
            reporter &&
            reporter->is_under_observation()) {
            // Send the whole object in case of resend
            m2m::Vector<uint16_t> changed_instance_ids;
            send_object_observation(object,
    ...
    if(!_base_list.empty()) {
        ...
        for ( ; base_iterator != _base_list.end(); base_iterator++ ) {
            if ((*base_iterator)->base_type() == M2MBase::Object) {
                handle_pending_notifications_for_object(clear, (M2MObject*)*base_iterator);
            }
        }
    }
}

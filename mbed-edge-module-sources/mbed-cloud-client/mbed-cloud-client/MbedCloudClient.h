...
public:
    ...
    /*
     * \brief Initiates the Cloud Client to reregister fully updating every resource.
     * Note: current subscriptions are cancelled. Therefore calling this method should be
     * avoided and it's preferred to use a method which just updates the current registration.
     */
    void reregister();
...
private:
    ...
    */
    ...
    /**
     * \brief Inspect if the given object pointer is already in object list
     * \param base The object pointer to compare
     * \return true if object is present in the objects list, false if not present
     */
